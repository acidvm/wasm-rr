...
pub fn sorted_headers(headers: &hyper::HeaderMap) -> HttpResult<Vec<(String, String)>> {
    let mut pairs = Vec::new();
    for (name, value) in headers.iter() {
        let value = value.to_str().map_err(|err| {
            HttpError::trap(wasmtime::format_err!(
...
pub fn header_map_from_pairs(pairs: &[(String, String)]) -> HttpResult<hyper::HeaderMap> {
    let mut map = hyper::HeaderMap::new();
    for (name, value) in pairs {
        let header_name =
            hyper::header::HeaderName::from_bytes(name.as_bytes()).map_err(|err| {
                HttpError::trap(wasmtime::format_err!("invalid header name {name}: {err}"))
            })?;
        let header_value = hyper::header::HeaderValue::from_str(value).map_err(|err| {
            HttpError::trap(wasmtime::format_err!(
